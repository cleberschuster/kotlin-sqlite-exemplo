...
class MainActivity : BaseActivity() {
    ...
    private fun onClickBuscar(){
        ...
        Thread(Runnable {
            Thread.sleep(500)
            var listaFiltrada: List<ContatosModel> = mutableListOf()
            try {
    ...
    private fun buscarTodosContatos(){
        ...
        Thread(Runnable {
            ...
            var listaFiltrada: List<ContatosModel> = mutableListOf()
            try {
                listaFiltrada = ContatoApplication.instance.helperDB?.mostrarTodosContatos() ?: mutableListOf()
            }catch (ex: Exception){
                ex.printStackTrace()
            }
            runOnUiThread {
                ...
                recyclerView.adapter = adapter
                progress.visibility = View.GONE
                Toast.makeText(this,"Buscando por todos os contatos",Toast.LENGTH_SHORT).show()
            }
        }).start()
    }
