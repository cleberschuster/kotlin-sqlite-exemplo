...
class HelperDB(
    ...
    fun buscarContatos(busca: String, isBuscaPorID: Boolean = false) : List<ContatosModel> {
        val db = readableDatabase ?: return mutableListOf()
        var lista = mutableListOf<ContatosModel>()
        var where: String?
        ...
        var cursor = db.query(TABLE_NAME,null, where, args,null,null, COLUMNS_NOME)
        if (cursor == null){
            db.close()
    ...
    fun mostrarTodosContatos() : List<ContatosModel> {
        val db = readableDatabase ?: return mutableListOf()
        var lista = mutableListOf<ContatosModel>()

        var cursor = db.query(TABLE_NAME,null, null, null,null,null, COLUMNS_NOME)
        if (cursor == null){
            db.close()
            return mutableListOf()
        }
    ...
    fun salvarContato(contato: ContatosModel) {
    ...
    fun updateContato(contato: ContatosModel) {
        val db = writableDatabase ?: return
